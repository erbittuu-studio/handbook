...
public struct SYSAssetPack: Codable, Equatable, Sendable {
    ...
    /// Not real content — a pack pushed through the real pipeline to verify it
    /// before the content it's standing in for goes live. Invisible to a
    /// Release build: never downloaded, never listed. See `SYSAssets.includeTestPacks`.
    public let test: Bool?

    public init(id: String, title: SYSLocalizedText? = nil, bundle: String,
                bytes: Int? = nil, sha256: String? = nil, required: Bool? = nil, test: Bool? = nil) {
        self.id = id
        self.title = title
        self.bundle = bundle
        self.bytes = bytes
...
public actor SYSAssets {
    ...
    /// Whether `test` packs are downloaded and listed at all. Defaults to
    /// Debug-only: a Release build never fetches or shows one, so new content
    /// can be pushed through the real pipeline and checked on a device before
    /// the content it's standing in for goes live. Set explicitly for a
    /// dedicated QA/staging build that isn't Debug but still needs to see them.
    public var includeTestPacks: Bool = {
        #if DEBUG
        return true
        #else
        return false
    ...
    /// This app's own packs, minus any `test` ones a Release build shouldn't
    /// see — what a screen should actually list, as opposed to `manifest?.items`,
    /// which is everything the manifest happens to name.
    public func visiblePacks(in manifest: SYSAssetManifest) -> [SYSAssetPack] {
        includeTestPacks ? manifest.items : manifest.items.filter { $0.test != true }
    }
    ...
    /// together — a learning pack's images, a prayer's text in three
    /// languages — rather than one blob. Same manifest, same download and
    /// verification as any other pack; only the last step differs.
    private func isArchive(_ pack: SYSAssetPack) -> Bool {
        (pack.bundle as NSString).pathExtension.lowercased() == "zip"
    }

    /// Where an archive pack unzips to. Named by id, not by the zip's
    /// filename, since what is inside is what callers actually read.
    public func localDirectory(for pack: SYSAssetPack) throws -> URL {
        try cacheDirectory().appendingPathComponent(pack.id, isDirectory: true)
    }

    public func isUnzipped(_ pack: SYSAssetPack) -> Bool {
        guard let url = try? localDirectory(for: pack) else { return false }
        var isDirectory: ObjCBool = false
        return fileManager.fileExists(atPath: url.path, isDirectory: &isDirectory) && isDirectory.boolValue
    }

    /// Downloads an archive pack if needed, verifies it, and unzips it —
    /// the zip-bundle counterpart to `ensure(_:)`. Safe to call again: an
    /// already-unzipped pack returns its folder immediately.
    @discardableResult
    public func ensureUnzipped(_ packID: String) async -> Result<URL, SYSContentError> {
        guard let manifest = cachedManifest ?? loadPersistedManifest() else {
            ...
            return await ensureUnzipped(packID)
        }
        guard let pack = manifest.items.first(where: { $0.id == packID }) else {
            return .failure(.missingRequiredPack(id: packID))
        }
        if isUnzipped(pack), let directory = try? localDirectory(for: pack) {
            return .success(directory)
        }
        if case let .failure(error) = await ensure(packID) {
            return .failure(error)
        }
        return unzip(pack)
    }

    /// Unzips a pack already downloaded and verified, into place atomically:
    /// staged under a sibling name first, so a process killed mid-unzip
    /// leaves only the `.staging` leftover, never a half-extracted folder
    /// that `isUnzipped` would mistake for a complete one.
    private func unzip(_ pack: SYSAssetPack) -> Result<URL, SYSContentError> {
        guard let zipURL = try? localURL(for: pack), let destination = try? localDirectory(for: pack) else {
            return .failure(.notConfigured)
        }
        let staging = destination.appendingPathExtension("staging")
        try? fileManager.removeItem(at: staging)
        do {
            try SYSZip.unzip(at: zipURL, to: staging)
        } catch {
            try? fileManager.removeItem(at: staging)
            SYSLogger.error("assets: could not unzip \(pack.id) — \(error)")
        ...
        do {
            ...
            try? fileManager.removeItem(at: staging)
            SYSLogger.error("assets: could not install \(pack.id) — \(error)")
            return .failure(.corrupt(pack: pack.id))
        }
    ...
    public func prepareRequired(
        ...
        // A pack is only really ready once an archive pack is unzipped, not merely
        // downloaded — a kill between those two steps must not look complete.
        func isReady(_ pack: SYSAssetPack) -> Bool {
            isArchive(pack) ? isUnzipped(pack) : isCached(pack)
        }

        let outstanding = wanted.filter { !isReady($0) }
        ...
        for pack in outstanding {
            // isCached rather than unconditionally fetching: an archive pack can already
            // be downloaded from an earlier run that was killed before it got to unzip —
            // that must not re-fetch bytes it already has.
            var downloadError: SYSContentError?
            if !isCached(pack), case let .failure(error) = await fetchOnce(pack) {
                downloadError = error
            }
            switch downloadError {
            case .none:
                if isArchive(pack), case let .failure(error) = unzip(pack) {
                    SYSLogger.error("assets: required pack \(pack.id) failed to unzip — \(error)")
                    return .failure(error)
                }
                completed += 1
    ...
    /// `ensure(_:)` for a pack the caller already knows about, rather than one
    /// looked up in this app's own catalog manifest — an item's own manifest
    /// entry already named its audio, say. Same download, verification and
    /// caching either way; this just skips the manifest fetch.
    public func ensure(pack: SYSAssetPack) async -> Result<Data, SYSContentError> {
        if let data = cachedData(for: pack) { return .success(data) }
        return await fetchOnce(pack)
    }
    ...
    private func requiredPacks(in manifest: SYSAssetManifest) -> [SYSAssetPack] {
        let candidates = visiblePacks(in: manifest)
        if !explicitRequired.isEmpty {
            return candidates.filter { explicitRequired.contains($0.id) }
        }
        return candidates.filter { $0.required == true }
    }
    ...
    public func pruneStalePacks() -> Int {
        ...
        // An archive pack leaves two entries — the zip, named by its content hash,
        // and the folder it unzipped to, named by id — both need keeping.
        let live = Set(manifest.items.map { ($0.bundle as NSString).lastPathComponent })
            .union(manifest.items.map(\.id))
        var removed = 0
        for entry in entries where !live.contains(entry) || entry.hasSuffix(".staging") {
