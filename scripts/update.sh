...
# An app opts in to the shared build settings with `xcconfig: true` in its project file. The files are
# not added to an app that has not switched its project over, or the sync would leave it failing.
[[ "$(project_value xcconfig)" == "true" ]] && USES_XCCONFIG="yes" || USES_XCCONFIG="no"
...
SEEDED=(
  ...
  "App/Config/App.xcconfig|xcconfig/App.xcconfig"
)
...
try:    print(json.load(open('$TARGET/.firebaserc'))['projects']['default'])
  ...
  BUNDLE_ID="$(project_value app.bundleId)"
  # The product name (store, release title, Xcode Cloud). Distinct from PROJECT_NAME, which is the
  # .xcodeproj's file name and may be the same for every app.
  APP_NAME="$(project_value app.name)"
  [[ -n "$APP_NAME" ]] || APP_NAME="$PROJECT_NAME"
  CONFIG_URL=""
  ...
  FIREBASE_VARIANT="$(project_value app.firebaseVariant)"
...
detect
current="$(project_value pes.version)"
[[ -n "$current" ]] || current="$(cat "$TARGET/.pes-version" 2>/dev/null | tr -d '[:space:]' || true)"
...
# In project.yml the baseline is a comment block at the end of the file (`# HASH  PATH` lines), read
# into a scratch file so the rest of this script sees the same format either way.
if [[ "$PROJECT_FILE" == "project.yml" ]]; then
  MANIFEST="$(mktemp)"; MANIFEST_NEW="$(mktemp)"
  awk '/^# >>> sync manifest/{f=1;next} /^# <<</{f=0} f{sub(/^# /,"");print}' "$TARGET/project.yml" > "$MANIFEST"
fi
...
for rel in "${MANAGED[@]}"; do
  [[ "$rel" == "App/Config/Base.xcconfig" && "$USES_XCCONFIG" == "no" ]] && continue
...
if [[ $CHECK -eq 0 ]]; then
  if [[ "$PROJECT_FILE" == "project.yml" ]]; then
    python3 - "$TARGET/project.yml" "$MANIFEST_NEW" <<'PY'
import re, sys
path, manifest = sys.argv[1], sys.argv[2]
text = open(path).read()
text = re.sub(r"\n?# >>> sync manifest.*?# <<<[^\n]*\n?", "\n", text, flags=re.S).rstrip("\n") + "\n"
rows = "".join("# " + line for line in open(manifest))
text += "\n# >>> sync manifest - written by update.sh, do not edit\n" + rows + "# <<<\n"
open(path, "w").write(text)
PY
    rm -f "$MANIFEST" "$MANIFEST_NEW"
  else
...
for entry in "${SEEDED[@]}"; do
  rel="${entry%%|*}"; src="${entry##*|}"
  [[ "$USES_SYS" == "no" && "$rel" == *AnalyticsManager.swift ]] && continue
  [[ "$USES_XCCONFIG" == "no" && "$rel" == *App.xcconfig ]] && continue
  # An app still on Project.json is not handed a second project file; migrate it first.
  [[ "$rel" == "project.yml" && -f "$TARGET/Project.json" ]] && continue
  [[ -f "$PES_ROOT/templates/$src" ]] || continue
...
if [[ -d "$TARGET/.github/workflows" ]]; then
  for f in "$TARGET/.github/workflows"/*.yml; do
    [[ -e "$f" ]] || continue
    name="$(basename "$f")"
    case "$name" in pr.yml|main.yml|release.yml|diagnose.yml|xcode-cloud.yml|release-guard.yml) ;;
...
if [[ $CHECK -eq 1 ]]; then
  ...
  if [[ "$PROJECT_FILE" == "project.yml" ]]; then
    python3 - "$TARGET/project.yml" "$PES_VERSION" <<'PY'
import re, sys
path, version = sys.argv[1], sys.argv[2]
text = open(path).read()
if re.search(r"^pes:\s*\n\s+version:", text, re.M):
    text = re.sub(r"(^pes:\s*\n\s+version:)[^\n]*", r"\1 " + version, text, count=1, flags=re.M)
else:
    text = "pes:\n  version: " + version + "\n\n" + text
open(path, "w").write(text)
PY
    echo "Updated to PES $PES_VERSION (project.yml). Review the diff and commit."
  else
