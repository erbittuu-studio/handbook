...
# It also records the PES version, so update.sh knows where this app started.
copy app/project.yml       project.yml
...
# under "app" in project.yml so update.sh keeps syncing the right one.
