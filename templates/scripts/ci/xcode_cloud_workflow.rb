...
PRODUCT = ARGV.index("--product")&.then { |i| ARGV[i + 1] } || "{{APP_NAME}}"
...
if check_target
  ...
  if File.file?(project_file)
    ...
    if expected && archive["scheme"] != expected
      abort "\n'#{check_target}' archives scheme #{archive['scheme'].inspect}, but project.yml says app.scheme is " \
            "#{expected.inspect} — the release build cannot find it. " \
            "Fix: Xcode → Product → Xcode Cloud → Manage Workflows → #{check_target} → Archive action → scheme."
    end
    puts "  archive scheme #{archive['scheme'].inspect} matches project.yml"
  end
  folders = (workflow.dig("attributes", "branchStartCondition", "filesAndFoldersRule", "matchers") || []).map { |m| m["directory"] }
  unless folders.include?(BUILD_DIRECTORY)
    abort "\n'#{check_target}' does not watch #{BUILD_DIRECTORY}/ (watches #{folders.compact.inspect}) — changes to the app would not start a build."
  end
