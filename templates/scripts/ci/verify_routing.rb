...
SCENARIOS = [
  { name: "code only",        files: ["App/Source/App/{{PROJECT_NAME}}App.swift"],
    build: true,  ci: false },
  ...
  { name: "workflow only",    files: [".github/workflows/main.yml"],
    build: false, ci: true },
  { name: "Project.json",     files: ["Project.json"],
    build: false, ci: true },
  { name: "code + CI script", files: ["App/Source/App/{{PROJECT_NAME}}App.swift", "scripts/ci/asc_builds.rb"],
    build: true,  ci: true }
].freeze
